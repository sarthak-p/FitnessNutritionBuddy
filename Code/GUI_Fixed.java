...
public class FitnessStart extends Application {
	...
	public void caloriesDisplay(Stage primaryStage) {
		...
		VBox title = new VBox(label);
		title.setAlignment(Pos.CENTER);
		
		Label calories = new Label (user.calorie + " Calories allowed");
		calories.setFont(new Font(20));
		Label carb = new Label(user.carb + " g of carbs allowed");
		carb.setFont(new Font(20));
		Label protein  = new Label(user.protein + " g of protein allowed");
		protein.setFont(new Font(20));
		Label fat = new Label(user.fat + " g of fat allowed");
		fat.setFont(new Font(20));
		
		VBox info = new VBox(20, calories, carb, protein, fat);
		...
		double progress = (double)(user.calorie - user.caloriesConsumed) / (double) user.calorie;
		progressBar.setProgress(progress);

		VBox center = new VBox(50, progressBar, info);
		...
		Scene scene = new Scene(borderPane, 600, 600, Color.MINTCREAM);
	...
	public void changeGoalScreen(Stage primaryStage) {
		...
		// set Calorie change goal
		Label set1 = new Label();
		set1.setFont(Font.font(15));

		Label calorie = new Label(user.calorie + " calories per day");
		calorie.setFont(Font.font(15));

		TextField calgoal = new TextField();
		calgoal.setPromptText("Enter new goal: ");

		Button calbutton = new Button("Set goal");
		calbutton.setOnAction(actionEvent -> {
			cal_goal = Integer.parseInt(calgoal.getText());
			if (cal_goal < 1000) {
				set1.setText("Goal is too low, try again");
			} else {
				set1.setText("Goal set!");
				user.calorie = cal_goal;
				calorie.setText("Current goal: " + user.calorie + " calories per day");
			}
		});

		HBox box1 = new HBox(10, calgoal, calbutton, set1);
		VBox v1 = new VBox(5, calorie, box1);


		// Set Carbohydrates change goal
		Label set2 = new Label();
		set2.setFont(Font.font(15));

		Label carb = new Label(user.carb + " g of carb per day");
		carb.setFont(Font.font(15));

		TextField carbgoal = new TextField();
		carbgoal.setPromptText("Enter new goal: ");

		Button carbbutton = new Button("Set goal");
		carbbutton.setOnAction(actionEvent -> {
			carb_goal = Integer.parseInt(carbgoal.getText());
			set2.setText("Goal set!");
			user.carb = carb_goal;
			carb.setText("Current goal: " + user.carb + " g of carb per day");
		});

		HBox box2 = new HBox(10, carbgoal, carbbutton, set2);
		VBox v2 = new VBox(5, carb, box2);


		// Set Fat change goal
		Label set3 = new Label();
		set3.setFont(Font.font(15));

		Label fat = new Label(user.fat + " g of fat per day");
		fat.setFont(Font.font(15));

		TextField fatgoal = new TextField();
		fatgoal.setPromptText("Enter new goal: ");

		Button fatbutton = new Button("Set goal");
		fatbutton.setOnAction(actionEvent -> {
			fat_goal = Integer.parseInt(fatgoal.getText());
			set3.setText("Goal set!");
			user.fat = fat_goal;
			fat.setText("Current goal: " + user.fat + " g of fat per day");
		});

		HBox box3 = new HBox(10, fatgoal, fatbutton, set3);
		VBox v3 = new VBox(5, fat, box3);


		// Set Protein change goal
		Label set4 = new Label();
		set4.setFont(Font.font(15));

		Label protein = new Label(user.protein + " g of protein per day");
		protein.setFont(Font.font(15));

		TextField proteingoal = new TextField();
		proteingoal.setPromptText("Enter new goal: ");

		Button probutton = new Button("Set goal");
		probutton.setOnAction(actionEvent -> {
			pro_goal = Integer.parseInt(proteingoal.getText());
			set4.setText("Goal set!");
			user.protein = pro_goal;
			protein.setText("Current goal: " + user.protein + " g of protein per day");
		});

		HBox box4 = new HBox(10, proteingoal, probutton, set4);
		VBox v4 = new VBox(5, protein, box4);

		// Set page layout
		VBox all = new VBox(20, v1, v2, v3, v4);
		all.setTranslateX(200);
		...
		Text title = new Text("Change Goals");
		title.setTranslateX(225);
		title.setTranslateY(30);
		title.setFont(Font.font("Arial", 25));

		
		HBox t = new HBox(title);
		t.setAlignment(Pos.CENTER);
		t.setPadding(new Insets(20, 20, 20, 20));

		// Create layout
		
		Group root = new Group(title, all, backButton);
	...
	public void nutritionScreen(Stage primaryStage) {
		...
		caloriesInput.setOnKeyPressed(e -> {
			if (e.getCode() == KeyCode.ENTER) {
				...
				user.caloriesConsumed = caloriesValue;
		...
		resetButton.setLayoutX(130);
		resetButton.setLayoutY(560);
	...
	// restaurant search results for the given city
	public void restaurantSearch(Stage primaryStage, TextField special, TextField location) {
		primaryStage.setTitle("Search");

		Text title = new Text("Resturant Search Results");
		title.setFont(Font.font("Arial", 20));
		BorderPane.setAlignment(title, Pos.CENTER);
		BorderPane.setMargin(title, new Insets(10, 10, 10, 30));

		VBox resultBox = new VBox();
		resultBox.setPadding(new Insets(20, 30, 30, 30));
		...
		ResturantInfo[] result = ResturantInfo.get_resturant_array();

		String k1 = "Keto";
		String k2 = "Chicago";

		// check if default needs to be used
		if(!special.getText().isBlank()) {
			k1 = special.getText();
		}
		...
		// find all the restaurants in the given city with the give specialty and add to resultBox
		for (ResturantInfo resturantInfo : result) {
			if (resturantInfo.specialty.toLowerCase().equals(k1.toLowerCase()) && resturantInfo.location.toLowerCase().equals(k2.toLowerCase())) {
				Text name_plus_location = new Text(resturantInfo.name + "\n" + resturantInfo.street + ",\n" + resturantInfo.location + ", " + resturantInfo.state_zip);
				Font f = new Font("Arial", 14);
				name_plus_location.setFont(f);
				resultBox.getChildren().add(name_plus_location);
			}
		}

		// Create back button
		Button backButton = new Button("Back");
		backButton.setOnAction(e -> restaurantScreen(primaryStage));
		...
		Scene scene = new Scene(scroll, 600, 600, Color.MINTCREAM);
		primaryStage.setScene(scene);
		primaryStage.show();

	}

	// Displays restaurants nearby with a given food specialty
	public void restaurantScreen(Stage primaryStage) {
		...
		searchField2.setPrefWidth(325);
		searchField2.setFont(Font.font("Arial", 14));
		searchField2.setPromptText("Enter City");
		HBox searchBox2 = new HBox(searchIcon2, searchField2);
		searchBox2.setAlignment(Pos.CENTER);
		searchBox2.setPadding(new Insets(20, 30, 20, 30));

		Button search = new Button("Search");
		HBox searchBox1 = new HBox(search);
		searchBox1.setAlignment(Pos.CENTER);
		searchBox1.setPadding(new Insets(0, 30, 20, 30));
		search.setOnAction(actionEvent -> {restaurantSearch(primaryStage, searchField, searchField2);});

		VBox centerBox = new VBox(searchBox, searchBox2, searchBox1);
		centerBox.setAlignment(Pos.CENTER);
		centerBox.setSpacing(10);

		// Set up the list of restaurants with default location and specialty
		VBox resultBox = new VBox();
		resultBox.setPadding(new Insets(10, 30, 30, 30));
		resultBox.setSpacing(10);

		Text resultTitle = new Text("Resturants Close by:");
		...
		for(int i = 0; i < result.length; i++) {
			if(result[i].specialty == "Keto" && result[i].location == "Chicago") {
				Text name_plus_location = new Text(result[i].name + "\n" + result[i].street + ",\n" +result[i].location + ", " + result[i].state_zip);
				Font f = new Font("Arial", 14);
				name_plus_location.setFont(f);
				resultBox.getChildren().add(name_plus_location);
			}
...
//		String image = "background_image.jpg";
