...
public class FitnessStart extends Application {
	...
	public void changeGoalScreen(Stage primaryStage) {
		...
		Label carb = new Label(user.carb + " carbs per day");
		...
		carbbutton.setOnAction(actionEvent -> {
			...
			user.carb = carb_goal;
			carb.setText("Current goal: " + user.carb + " carbs per day");
		});
		...
		Label fat = new Label(user.fat + " fats per day");
		...
		fatbutton.setOnAction(actionEvent -> {
			...
			user.fat = fat_goal;
			fat.setText("Current goal: " + user.fat + " fats per day");
		});
		...
		Label protein = new Label(user.protein + " proteins per tain");
		...
		probutton.setOnAction(actionEvent -> {
			...
			protein.setText("Current goal: " + user.protein + " proteins per day");
		});
	...
	for(int i = 0; i < result.length; i++) {
		if(result[i].specialty.toLowerCase().equals(k1.toLowerCase()) && result[i].location.toLowerCase().equals(k2.toLowerCase())) {
	...
	// Displays restaurants near by with a given food specialty
